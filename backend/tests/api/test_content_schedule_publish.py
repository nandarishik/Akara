...
from app.infra.content.cms_service import publish_content, publish_due_scheduled_content
...
def test_scheduled_content_deferred_until_due(_mock_supa):
    ...
    with patch("app.infra.content.cms_service.get_content_entry", return_value=entry):
        result = publish_content("landing.hero.title", force=False)
        assert result.get("deferred") is True
        assert result.get("scheduled_at") == future


@patch("app.infra.content.cms_service.publish_content")
@patch("app.infra.content.cms_service.get_supabase_service_client")
